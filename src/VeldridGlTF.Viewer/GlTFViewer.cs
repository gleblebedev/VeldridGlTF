...
using Veldrid;
using VeldridGlTF.Viewer.Objects;

namespace VeldridGlTF.Viewer
{
    public class GlTFViewer
    {
        ...
        private void AddGltfObjects()
        {
            var model = ModelRoot.Load("../../../../modules/glTF-Sample-Models/2.0/Buggy/glTF-Binary/Buggy.glb");
            var meshes = new Dictionary<Mesh, List<ConstructedMeshInfo>>();
            foreach (var mesh in model.LogicalMeshes)
            {
                var m = ConstructMesh(mesh);
                if (m != null)
                {
                    meshes.Add(mesh,m);
                }
            ...
            foreach (var node in model.LogicalNodes)
            {
                if (node.Mesh != null)
                {
                    List<ConstructedMeshInfo> meshInfos;
                    if (meshes.TryGetValue(node.Mesh, out meshInfos))
                    {
                        //TODO: calc world transform
                        var tm = node.GetWorldMatrix(null,0);
                        Vector3 scale;
                        Quaternion rot;
                        Vector3 tr;
                        Matrix4x4.Decompose(tm, out scale, out rot, out tr);
        ...
        private List<ConstructedMeshInfo> ConstructMesh(Mesh mesh)
        {
            var meshInfos = new List<ConstructedMeshInfo>();
            ...
            foreach (var meshPrimitive in mesh.Primitives)
            {
                ...
                {
                    var pos = positions[i];
                    var norm = (normals != null) ? normals.Value[i] : Vector3.UnitX;
                    var uv = (texCoords0 != null) ? texCoords0.Value[i] : Vector2.Zero;
                    var vertexPositionNormalTexture = new VertexPositionNormalTexture(pos, norm, uv);
                    vertices.Add(vertexPositionNormalTexture);
                }

                foreach (var face in meshPrimitive.GetTriangleIndices())
                {
                    indices.Add((ushort) face.Item3);
                    indices.Add((ushort) face.Item2);
                    indices.Add((ushort) face.Item1);
                }
                meshInfos.Add(new ConstructedMeshInfo(vertices.ToArray(), indices.ToArray(), meshPrimitive.Material?.Name));
            }
